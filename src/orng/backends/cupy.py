...
class CuPyBackend:
    ...
    def load_state_dict(self, state: Mapping[str, Any]) -> None:
        try:
            ...
            raise ValueError(
                "CuPy RNG state requires 'bit_generator' and "
                "'bit_generator_state'."
            ) from exc
        bit_generator_cls = getattr(
            self._impl._cupy.random, bit_generator_name, None
        )
        if bit_generator_cls is None:
            raise ValueError(
                f"Unknown CuPy bit generator '{bit_generator_name}'."
            )
        bit_generator = bit_generator_cls()
        bit_generator.state = copy.deepcopy(bit_generator_state)
        self._state = self._impl._cupy.random.Generator(bit_generator)
